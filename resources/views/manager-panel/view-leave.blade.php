...
@section('content')
<x-breadcrumb :items="[
    ['label' => 'My Leaves', 'url' => route('hr.manager-panel.my-leaves')],
    ['label' => 'Details'],
]" />
        ...
        <div class="mt-6 flex flex-col gap-3 sm:flex-row">
            @if ($leave->status === 'pending')
                <form method="POST" action="{{ route('hr.manager-panel.cancel-leave', $leave->id) }}" onsubmit="return confirm('Cancel this leave?')">
                    @csrf
                    @method('DELETE')
                    <x-btn type="submit" color="red" class="w-full sm:w-auto">Cancel Request</x-btn>
                </form>
            @endif
            <x-btn-outline href="{{ route('hr.manager-panel.my-leaves') }}" class="w-full sm:w-auto">
                Back to Leaves
            </x-btn-outline>
        </div>
    </x-card>
