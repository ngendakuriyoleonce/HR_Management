...
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <x-btn href="{{ route('hr.manager-panel.request-leave') }}" color="emerald" class="w-full sm:w-auto">
                                ...
                                <tr>
                                    ...
                                    <td class="py-3">
                                        <div class="flex items-center gap-1.5">
                                            <a href="{{ route('hr.manager-panel.view-leave', $leave->id) }}" class="inline-flex items-center gap-1 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-100 transition-colors">
                                                View
                                            </a>
                                            @if ($leave->status === 'pending')
                                                <form method="POST" action="{{ route('hr.manager-panel.cancel-leave', $leave->id) }}" onsubmit="return confirm('Cancel this leave?')">
                                                    @csrf
                                                    @method('DELETE')
